...
DatabaseCache.prototype = {
    ...
    store(key, value) {
        ...
        if (type === 0) {
            this._store[key] = new User(data.id, data.name, data.email, data.hash, data.salt, data.checkIns);
            return this._store[key];
        } else if (type === 1) {
            this._store[key] = new Business(data.id, data.name, data.email, data.hash, data.salt, data.checkIns, data.location, data.category, data.tags);
            return this._store[key];
        } else {
            return -1;
        }
